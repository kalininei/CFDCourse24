...
private:
	...
	const double _delta_nuT = 0.01;   // nuT >= delta_nuT * 1/Re
	...
	_grad_computer(grid, _collocations)
{
	...
	_nuT = std::vector<double>(vec_size(), _delta_nuT / _Re);
...
std::vector<double> Cavity2DFvmSimpleWorker_KEps::compute_nuT() const{
	// TODO
	return std::vector<double>(vec_size(), _delta_nuT/_Re);
}
...
std::vector<double> Cavity2DFvmSimpleWorker_KEps::compute_k(const std::vector<double>& G, const std::vector<double>& gamma) const{
	...
	std::vector<double> diag(vec_size());
	for (size_t i=0; i<diag.size(); ++i) diag[i] = _tau * gamma[i];
	CsrMatrix mat = assemble_convdiff_lhs(_k, nu, diag);
...
std::vector<double> Cavity2DFvmSimpleWorker_KEps::compute_epsilon(const std::vector<double>& G, const std::vector<double>& gamma) const{
	...
	CsrMatrix mat = assemble_convdiff_lhs(_epsilon, nu, std::vector<double>(vec_size(), _tau * _C2));
...
TEST_CASE("Cavity 2D with k-e model, FVM-SIMPLE-Laminar algorithm", "[cavity-fvm-rans-ke]"){
	std::cout << std::endl << "--- cfd24_test [cavity-fvm-rans-ke] --- " << std::endl;
...
struct Cavity2DFvmSimpleWorker_KOmega{
	Cavity2DFvmSimpleWorker_KOmega(const IGrid& grid, double Re, double E);
	void initialize_saver(std::string stem);
	double step();
	void save_current_fields(size_t iter);
...
private:
	...
	const double _Cmu = 0.09;
	const double _sigma_k = 2.0;
	const double _sigma_o = 2.0;
	const double _beta = 3.0/40;
	const double _gamma = 5.0/9;
	const double _delta_nuT = 0.01;   // nuT >= delta_nuT * 1/Re
	...
	std::vector<double> compute_G() const;
	std::vector<double> compute_nuT() const;
	std::vector<double> compute_omega(const std::vector<double>& G, const std::vector<double>& omega) const;
	std::vector<double> compute_k(const std::vector<double>& G, const std::vector<double>& omega) const;

	static double compute_tau(const IGrid& grid, double Re, double E);
};

double Cavity2DFvmSimpleWorker_KOmega::compute_tau(const IGrid& grid, double Re, double E){
	double h2 = grid.cell_volume(0);
	for (size_t i=1; i<grid.n_cells(); ++i){
		h2 = std::max(h2, grid.cell_volume(i));
	}
...
Cavity2DFvmSimpleWorker_KOmega::Cavity2DFvmSimpleWorker_KOmega(const IGrid& grid, double Re, double E):
	_grid(grid),
	_Re(Re),
	_tau(compute_tau(grid, Re, E)),
	_alpha_p(1.0/(1.0 + E)),
	...
	_grad_computer(grid, _collocations)
{
	...
	_p = std::vector<double>(vec_size(), 0);
	_k = std::vector<double>(vec_size(), 0);
	_omega = std::vector<double>(vec_size(), 0);
	_nuT = std::vector<double>(vec_size(), _delta_nuT / _Re);
...
void Cavity2DFvmSimpleWorker_KOmega::gather_boundary_collocations(){
	BoundaryInfo& bi = _boundary_info;
...
void Cavity2DFvmSimpleWorker_KOmega::initialize_saver(std::string stem){
	_writer.reset(new VtkUtils::TimeSeriesWriter(stem));
};

double Cavity2DFvmSimpleWorker_KOmega::to_next_iteration(){
	assemble_uv_slae();
...
double Cavity2DFvmSimpleWorker_KOmega::step(){
	// Predictor step: U-star
	std::vector<double> u_star = compute_u_star();
	std::vector<double> v_star = compute_v_star();
	std::vector<double> un_star_face = compute_un_star_face_rhie_chow(u_star, v_star);
	...
	std::vector<double> G = compute_G();
	// omega
	_omega = compute_omega(G, _omega);
	// k
	_k = compute_k(G, _omega);
	// turbulent viscosity
	_nuT = compute_nuT();
...
void Cavity2DFvmSimpleWorker_KOmega::save_current_fields(size_t iter){
	if (_writer){
		std::string filepath = _writer->add(iter);
		_grid.save_vtk(filepath);
		VtkUtils::add_cell_data(_p, "pressure", filepath, _grid.n_cells());
		VtkUtils::add_cell_vector(_u, _v, "velocity", filepath, _grid.n_cells());
		VtkUtils::add_cell_data(_k, "k", filepath, _grid.n_cells());
		VtkUtils::add_cell_data(_omega, "omega", filepath, _grid.n_cells());
	}
}

void Cavity2DFvmSimpleWorker_KOmega::assemble_p_stroke_solver(){
	LodMatrix mat(vec_size());
	for (size_t iface = 0; iface < _grid.n_faces(); ++iface){
		size_t negative_colloc = _collocations.tab_face_colloc(iface)[0];
		size_t positive_colloc = _collocations.tab_face_colloc(iface)[1];
...
CsrMatrix Cavity2DFvmSimpleWorker_KOmega::assemble_convdiff_lhs(
		const std::vector<double>& u,
		const std::vector<double>& nu_face,
		const std::vector<double>& diag) const{
	std::vector<Vector> grad_u = _grad_computer.compute(u);
...
void Cavity2DFvmSimpleWorker_KOmega::assemble_uv_slae(){
	std::vector<double> nu = compute_nueff_face();
	_mat_u = assemble_convdiff_lhs(_u, nu);
	_mat_v = assemble_convdiff_lhs(_v, nu);
...
std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_u_star(){
	std::vector<double> u_star(_u);
	_u_solver.solve(_rhs_u, u_star);
	return u_star;
}

std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_v_star(){
	std::vector<double> v_star(_v);
	_v_solver.solve(_rhs_v, v_star);
	return v_star;
}

std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_un_star_face_rhie_chow(
		const std::vector<double>& u_star,
		const std::vector<double>& v_star){
...
std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_un_stroke_face(const std::vector<double>& dpstroke_dn){
	std::vector<double> un(_grid.n_faces());
	for (size_t iface=0; iface<_grid.n_faces(); ++iface){
		un[iface] = -_tau * _d * dpstroke_dn[iface];
	}
...
std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_p_stroke(const std::vector<double>& un_star_face){
	std::vector<double> rhs(vec_size(), 0.0);
	// internal
	for (size_t iface = 0; iface < _grid.n_faces(); ++iface){
		double coef = -_grid.face_area(iface) / _tau * un_star_face[iface];
...
std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_u_stroke(const std::vector<Vector>& grad_p_stroke){
	std::vector<double> u_stroke(vec_size(), 0.0);
	for (size_t i=0; i<_grid.n_cells(); ++i){
		u_stroke[i] = -_tau * _d * grad_p_stroke[i].x();
	}
...
std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_v_stroke(const std::vector<Vector>& grad_p_stroke){
	std::vector<double> v_stroke(vec_size(), 0.0);
	for (size_t i=0; i<_grid.n_cells(); ++i){
		v_stroke[i] = -_tau * _d * grad_p_stroke[i].y();
	}
...
std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_nueff_face(double sigma) const{
	// TODO: nueff_face = 1/Re  + nuT_face / sigma
	return std::vector<double>(_grid.n_faces(), 1.0/_Re);
}

std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_G() const{
	// TODO
	return std::vector<double>(vec_size(), 0.0);
};

std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_nuT() const{
	// TODO
	return std::vector<double>(vec_size(), _delta_nuT / _Re);
}

std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_k(const std::vector<double>& G, const std::vector<double>& omega) const{
	// left side
	std::vector<double> nu = compute_nueff_face(_sigma_k);
	std::vector<double> diag(vec_size());
	for (size_t i=0; i<diag.size(); ++i){
		diag[i] = _tau*_Cmu*omega[i];
	}
	CsrMatrix mat = assemble_convdiff_lhs(_k, nu, diag);
...
std::vector<double> Cavity2DFvmSimpleWorker_KOmega::compute_omega(const std::vector<double>& G, const std::vector<double>& omega) const{
	std::vector<double> nu = compute_nueff_face(_sigma_o);
	std::vector<double> diag(vec_size());
	for (size_t i=0; i<diag.size(); ++i){
		diag[i] = _tau*_beta*omega[i];
	}
	CsrMatrix mat = assemble_convdiff_lhs(_omega, nu, diag);
	
	// ============== right side
	std::vector<double> rhs(vec_size(), 0.0);
	for (size_t icell = 0; icell < _grid.n_cells(); ++icell){
		double v = _gamma * G[icell] / _nuT[icell];
		rhs[icell] = (_omega[icell] +_tau * v) * _grid.cell_volume(icell);
	}
	...
	// solve
	std::vector<double> new_o(_omega);
	AmgcMatrixSolver::solve_slae(mat, rhs, new_o);
	return new_o;
}

}

TEST_CASE("Cavity 2D with k-omega model, FVM-SIMPLE-Laminar algorithm", "[cavity-fvm-rans-ko]"){
	std::cout << std::endl << "--- cfd24_test [cavity-fvm-rans-ko] --- " << std::endl;
	...
	// worker initialization
	RegularGrid2D grid(0, 1, 0, 1, 30, 30);
	Cavity2DFvmSimpleWorker_KOmega worker(grid, Re, E);
	worker.initialize_saver("cavity-fvm-rans-ko");
