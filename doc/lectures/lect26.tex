...
\begin{aligned}
& x_k = x_{k-1} + \alpha_k p'_k + \omega_k s'_k, \\
...
для невязки имеет однородные граничные условия, то этот обмен сводится к заполнению нулями слоя фиктивных ячеек.
...
\item сравить локальные решения (то есть примерить оператор расширения со взвешиванием $\mat R_i^T \mat D_i$. Это делается методом
...
\item Как отмечалось ранее, применение общего оператора $\mat A$ к локально заданному вектору $u$
...
Оператор $\mat A$ применяется к векторам $p'$ и $s'$. После вычисления они должны быть обработаны этой процедурой.
